...
├── prisma/                          # schema·seed (마이그레이션 디렉토리 없음 — db push)
...
| 항목 | 내용 | 영향 범위 | 관련 spec |
|---|---|---|---|
| 마이그레이션 부재 | `prisma/migrations/` 없이 `db push`로 스키마 동기화. CI도 db push 사용 | 스키마 변경·CI | SPEC-001/T003·T018 |
| 통합 테스트 mock 차수 | service 테스트는 Prisma·S3·Resend mock. 실제 DB 트랜잭션·unique 동작은 CI E2E로만 간접 검증 | file·notification | SPEC-001/T016 |
| staging 미검증 | 실제 S3 업로드·만료 링크·Resend 발송·secret 노출은 미검증 | 파일·알림 | SPEC-001/T019 |
| S3 CORS 의존 | 브라우저 직접 PUT은 버킷 PUT CORS 설정 필요. 미설정 시 업로드 실패 | 파일 업로드 | SPEC-001/T012·T019 |
| Account unique 부재 | `Account`에 `@@unique([providerId,accountId])` 없음 → 중복 레코드 가능성 | 인증 | SPEC-001/T004 |
